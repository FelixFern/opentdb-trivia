...
export const useQuizPage = () => {
  ...
  const {
    data: quizData,
    refetch: handleRefetchQuiz,
    isFetching: isQuizLoading,
    isError: isQuizError,
  } = useQuizData({
  ...
  const normalizeQuiz = useMemo(() => {
    ...
    return quizResult?.map((quiz) => ({
      ...quiz,
      answer: [quiz.correct_answer, ...quiz.incorrect_answers].sort(() =>
        params?.type === QuizType.TrueFalse ? 1 : Math.random() - 0.5
      ),
    }));
  ...
  const handleAnswerClick = (selectedOption: string) => {
    ...
    setTimeout(() => {
      if (quizStatus.currentQuestion + 1 < (normalizeQuiz ?? []).length) {
        ...
        _handlePlaySound(QuizStatus.Complete);
