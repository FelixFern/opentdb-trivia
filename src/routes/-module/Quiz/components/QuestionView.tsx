    ...
    <>
      <div className="mb-6">
        <div className="flex flex-col items-center text-sm text-gray-500 mb-2">
          ...
          <span className="text-indigo-600">({decodeHtmlEntities(currentQuestion?.category ?? '')})</span>
        </div>
        ...
        {currentQuestion?.answer.map((option, index) => (
          <button
            ...
            className={`w-full py-3 rounded-lg text-lg font-medium transition
                  ${quizStatus.selectedAnswer === null ? 'bg-indigo-100 hover:bg-indigo-200 text-indigo-800' : ''}
                  ${
          ...
          >
            ...
            {getAnswerStatus(option) === QuizStatus.Correct && (
              <CheckCircle className="inline ml-2" size={20} data-testid="icon-correct-answer" />
            )}
            {getAnswerStatus(option) === QuizStatus.Incorrect && (
              <XCircle className="inline ml-2" size={20} data-testid="icon-wrong-answer" />
            )}
          </button>
        ))}
      </div>
